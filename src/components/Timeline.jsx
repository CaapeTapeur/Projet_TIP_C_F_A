import React from 'react';
import { Calendar } from 'lucide-react';
import { useTranslation } from '../App';

const Timeline = () => {
    const { t, currentLang } = useTranslation();

    const timelineYears = [1933, 1936, 1939, 1945, 1961, 1989, 1990, 2005];

    return (
        <section className="bg-gray-100 py-16 min-h-screen">
            <div className="container mx-auto px-4">
                <h2 className="text-4xl font-bold mb-12 text-center">
                    {t('ui.navigation.timeline')}
                </h2>
                <div className="relative max-w-4xl mx-auto">
                    {/* Ligne verticale */}
                    <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-gray-900 via-red-900 to-gray-900"></div>

                    {timelineYears.map((year, index) => (
                        <div
                            key={year}
                            className={`mb-8 flex justify-between items-center w-full ${index % 2 === 0 ? 'flex-row-reverse' : ''}`}
                        >
                            <div className="w-5/12"></div>

                            {/* Point sur la timeline */}
                            <div className="z-20 flex items-center justify-center w-12 h-12 bg-red-800 rounded-full shadow-lg ring-4 ring-white">
                                <Calendar size={24} className="text-white" />
                            </div>

                            {/* Carte avec l'événement */}
                            <div className={`w-5/12 ${index % 2 === 0 ? 'text-right' : 'text-left'}`}>
                                <div className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
                                    <h3 className="text-2xl font-bold text-red-900 mb-2">{year}</h3>
                                    {/* Utilise la clé de l'année pour récupérer la traduction */}
                                    <p className="text-gray-700">{t(`timeline.${year}`)}</p>
                                </div>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </section>
    );
};

export default Timeline;